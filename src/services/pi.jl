...
dimension db.sql.statementcassociated with this ID. This operation is useful because
...
- `identifier`: An immutable, Amazon Web Services Region-unique identifier for a data
  source. Performance Insights gathers metrics from this data source. To use a DB instance as
  a data source, specify its DbiResourceId value. For example, specify
  db-FAIHNTYBKTGAUSUZQYPDS2GW4A.
...
- `metric_types`: The types of metrics to return in the response. Valid values in the array
  include the following:    os (OS counter metrics)    db (DB load metrics)    db.sql.stats
  (per-SQL metrics)    db.sql_tokenized.stats (per-SQL digest metrics)
- `service_type`: The Amazon Web Services service for which Performance Insights returns
  metrics.
