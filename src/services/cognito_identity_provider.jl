...
- `"ClientMetadata"`: A map of custom key-value pairs that you can provide as input for any
  ...
  Developer Guide.  When you use the ClientMetadata parameter, remember that Amazon Cognito
  won't do the following:   Store the ClientMetadata value. This data is available only to
  Lambda triggers that are assigned to a user pool to support custom workflows. If your user
  pool configuration doesn't include triggers, the ClientMetadata parameter serves no
  purpose.   Validate the ClientMetadata value.   Encrypt the ClientMetadata value. Don't use
  Amazon Cognito to provide sensitive information.
"""
...
phone number before you can send SMS messages to U.S. phone numbers. If you use SMS text
messages in Amazon Cognito, you must register a phone number with Amazon Pinpoint. Amazon
Cognito will use the registered number automatically. Otherwise, Amazon Cognito users that
must receive SMS messages might be unable to sign up, activate their accounts, or sign in.
If you have never used SMS text messages with Amazon Cognito or any other Amazon Web
Service, Amazon Simple Notification Service might place your account in SMS sandbox. In
sandbox mode , you will have limitations, such as sending messages only to verified phone
...
Pools in the Amazon Cognito Developer Guide.  This message is based on a template that you
configured in your call to create or update a user pool. This template includes your custom
sign-up instructions and placeholders for user name and temporary password. Alternatively,
you can call AdminCreateUser with SUPPRESS for the MessageAction parameter, and Amazon
Cognito won't send any email.  In either case, the user will be in the
FORCE_CHANGE_PASSWORD state until they sign in and change their password.  AdminCreateUser
...
provider. If the user to disable is a Amazon Cognito User Pools native username + password
user, they aren't permitted to use their password to sign in. If the user to deactivate is
a linked external identity provider (IdP) user, any link between that user and an existing
user is removed. The next time the external user (no longer attached to the previously
linked DestinationUser) signs in, they must create a new user account. See
...
ProviderAttributeName must always be Cognito_Subject for social identity providers. The
...
Amazon Cognito will use the registered number automatically. Otherwise, Amazon Cognito
...
Amazon Web Service, Amazon Simple Notification Service might place your account in SMS
sandbox. In  sandbox mode , you will have limitations, such as sending messages only to
verified phone numbers. After testing in the sandbox environment, you can move out of the
SMS sandbox and into production. For more information, see  SMS message settings for Amazon
...
account to an external federated user identity that has not yet been used to sign in. You
can then use the federated user identity to sign in as the existing user account.   For
...
they sign in as the existing user account.  The maximum number of federated identities
...
application owner.  This action is administrative and requires developer credentials.

# Arguments
- `destination_user`: The existing user in the user pool to be linked to the external
  identity provider user account. Can be a native (Username + Password) Amazon Cognito User
  Pools user or a federated user (for example, a SAML or Facebook user). If the user doesn't
  exist, an exception is thrown. This is the user that is returned when the new user (with
  the linked identity provider attribute) signs in. For a native username + password user,
  the ProviderAttributeValue for the DestinationUser should be the username in the user pool.
  For a federated user, it should be the provider-specific user_id. The ProviderAttributeName
  of the DestinationUser is ignored. The ProviderName should be set to Cognito for users in
  Cognito user pools.  All attributes in the DestinationUser profile must be mutable. If you
  have assigned the user any immutable custom attributes, the operation won't succeed.
- `source_user`: An external identity provider account for a user who doesn't exist yet in
  the user pool. This user must be a federated user (for example, a SAML or Facebook user),
  not another native user. If the SourceUser is using a federated social identity provider,
  such as Facebook, Google, or Login with Amazon, you must set the ProviderAttributeName to
  Cognito_Subject. For social identity providers, the ProviderName will be Facebook, Google,
  or LoginWithAmazon, and Amazon Cognito will automatically parse the Facebook, Google, and
  Login with Amazon tokens for id, sub, and user_id, respectively. The ProviderAttributeValue
  for the user must be the same value as the id, sub, or user_id value found in the social
  identity provider token.  For SAML, the ProviderAttributeName can be any value that matches
  a claim in the SAML assertion. If you want to link SAML users based on the subject of the
  SAML assertion, you should map the subject to a claim through the SAML identity provider
  and submit that claim name as the ProviderAttributeName. If you set ProviderAttributeName
  to Cognito_Subject, Amazon Cognito will automatically parse the default unique identifier
...
must register a phone number with Amazon Pinpoint. Amazon Cognito will use the registered
number automatically. Otherwise, Amazon Cognito users that must receive SMS messages might
be unable to sign up, activate their accounts, or sign in. If you have never used SMS text
messages with Amazon Cognito or any other Amazon Web Service, Amazon Simple Notification
Service might place your account in SMS sandbox. In  sandbox mode , you will have
limitations, such as sending messages only to verified phone numbers. After testing in the
...
- `"ChallengeResponses"`: The challenge responses. These are inputs corresponding to the
  ...
  any other required attributes, USERNAME, SECRET_HASH (if app client is configured with
  client secret).     MFA_SETUP requires USERNAME, plus you must use the session value
  returned by VerifySoftwareToken in the Session parameter.   The value of the USERNAME
  attribute must be the user's actual username, not an alias (such as an email address or
  phone number). To make this simpler, the AdminInitiateAuth response includes the actual
  username value in the USERNAMEUSER_ID_FOR_SRP attribute. This happens even if you specified
...
- `"ClientMetadata"`: A map of custom key-value pairs that you can provide as input for any
  ...
  Amazon Cognito invokes any functions that are assigned to the following triggers: pre
  sign-up, custom message, post authentication, user migration, pre token generation, define
  auth challenge, create auth challenge, and verify auth challenge response. When Amazon
...
will use the registered number automatically. Otherwise, Amazon Cognito users that must
receive SMS messages might be unable to sign up, activate their accounts, or sign in. If
you have never used SMS text messages with Amazon Cognito or any other Amazon Web Service,
Amazon Simple Notification Service might place your account in SMS sandbox. In  sandbox
mode , you will have limitations, such as sending messages only to verified phone numbers.
...
- `"ClientMetadata"`: A map of custom key-value pairs that you can provide as input for any
  ...
  with Lambda Triggers in the Amazon Cognito Developer Guide.  When you use the
...
expiry. Access and Id tokens expire one hour after they're issued. Calling this action
requires developer credentials.
...
doesn't subsequently verify the software token, their account is set up to authenticate
without MFA. If MFA config is set to Optional at the user pool level, the user can then log
in without MFA. However, if MFA is set to Required for the user pool, the user is asked to
set up a new software token MFA during sign-in.
...
- `"Precedence"`: A non-negative integer value that specifies the precedence of this group
  ...
  with higher ornull Precedence values. If a user belongs to two or more groups, it is the
...
- `provider_details`: The identity provider details. The following list describes the
  ...
  client_id   client_secret   authorize_scopes     For Facebook:   client_id   client_secret
   ...
   private_key   authorize_scopes     For OpenID Connect (OIDC) providers:   client_id
  client_secret   attributes_request_method   oidc_issuer   authorize_scopes   authorize_url
  if not available from discovery URL specified by oidc_issuer key    token_url if not
  available from discovery URL specified by oidc_issuer key    attributes_url if not
  available from discovery URL specified by oidc_issuer key    jwks_uri if not available from
  discovery URL specified by oidc_issuer key    attributes_url_add_attributes a read-only
  property that is set automatically      For SAML providers:   MetadataFile OR MetadataURL
  IDPSignout (optional)
...
require you to register an origination phone number before you can send SMS messages to
U.S. phone numbers. If you use SMS text messages in Amazon Cognito, you must register a
phone number with Amazon Pinpoint. Amazon Cognito will use the registered number
automatically. Otherwise, Amazon Cognito users that must receive SMS messages might be
...
- `"AccessTokenValidity"`: The time limit, between 5 minutes and 1 day, after which the
  access token is no longer valid and can't be used. If you supply a TokenValidityUnits
...
- `"AnalyticsConfiguration"`: The Amazon Pinpoint analytics configuration for collecting
  metrics for this user pool.  In Amazon Web Services Regions where isn't available, User
  Pools only supports sending events to Amazon Pinpoint projects in Amazon Web Services
  Region us-east-1. In Regions where is available, User Pools will support sending events to
...
- `"ExplicitAuthFlows"`: The authentication flows that are supported by the user pool
  ...
  names with the ALLOW_ prefix. Note that values with ALLOW_ prefix must be used only along
  with the ALLOW_ prefix. Valid values include:    ALLOW_ADMIN_USER_PASSWORD_AUTH: Enable
  admin based user password authentication flow ADMIN_USER_PASSWORD_AUTH. This setting
...
- `"IdTokenValidity"`: The time limit, between 5 minutes and 1 day, after which the access
  token is no longer valid and can't be used. If you supply a TokenValidityUnits value, you
...
- `"SupportedIdentityProviders"`: A list of provider names for the identity providers that
  ...
  LoginWithAmazon.
- `"TokenValidityUnits"`: The units in which the validity times are represented. Default
...
- `"WriteAttributes"`: The user pool attributes that the app client can write to. If your
  ...
  when it tries to update the attribute. For more information, see Specifying Identity
...
might generate an SMS text message. Starting June 1, 2021, US telecom carriers require you
to register an origination phone number before you can send SMS messages to U.S. phone
numbers. If you use SMS text messages in Amazon Cognito, you must register a phone number
with Amazon Pinpoint. Amazon Cognito will use the registered number automatically.
Otherwise, Amazon Cognito users that must receive SMS messages might be unable to sign up,
...
your account in SMS sandbox. In  sandbox mode , you will have limitations, such as sending
messages only to verified phone numbers. After testing in the sandbox environment, you can
...
- `auth_flow`: The authentication flow for this call to run. The API action will depend on
  ...
  if it doesn't find the USERNAME in the user pool.     ADMIN_NO_SRP_AUTH isn't a valid value.
- `client_id`: The app client ID.
...
- `"Filter"`: A filter string of the form \"AttributeName Filter-Type \"AttributeValue\"\".
  ...
  The server-side filter matches no more than 1 attribute. For an advanced search, use a
...
- `"ChallengeResponses"`: The challenge responses. These are inputs corresponding to the
  ...
  when signing in with a remembered device.     NEW_PASSWORD_REQUIRED: NEW_PASSWORD, any
  other required attributes, USERNAME.     SOFTWARE_TOKEN_MFA: USERNAME and
...
- `"MfaConfiguration"`: The MFA configuration. If you set the MfaConfiguration value to
  ‘ON’, only users with an MFA factor set up can sign in. To learn more, see Adding
...
- `"LambdaConfig"`: The Lambda configuration information from the request to update the
  user pool.
- `"MfaConfiguration"`: Can be one of the following values:    OFF - MFA tokens aren't
  required and can't be specified during user registration.    ON - MFA tokens are required
  for all user registrations. You can only specify ON when you're initially creating a user
...
- `"AnalyticsConfiguration"`: The Amazon Pinpoint analytics configuration for collecting
  metrics for this user pool.  In Amazon Web Services Regions where isn't available, User
  Pools only supports sending events to Amazon Pinpoint projects in us-east-1. In Regions
  where Pinpoint is available, User Pools will support sending events to Amazon Pinpoint
...
- `"ExplicitAuthFlows"`: The authentication flows that are supported by the user pool
  ...
  protocol to verify passwords.    ALLOW_USER_SRP_AUTH: Enable SRP-based authentication.
  ALLOW_REFRESH_TOKEN_AUTH: Enable authflow to refresh tokens.
- `"IdTokenValidity"`: The time limit after which the ID token is no longer valid and can't
  be used.
- `"LogoutURLs"`: A list of allowed logout URLs for the identity providers.
- `"PreventUserExistenceErrors"`: Errors and responses that you want Amazon Cognito APIs to
  return during authentication, account confirmation, and password recovery when the user
...
- `"TokenValidityUnits"`: The units in which the validity times are represented. Default
  for RefreshToken is days, and default for ID and access tokens is hours.
...
# Arguments
- `access_token`: The access token of the request to verify user attributes.
